 ...
 */ 
 ...
 class Imports extends MY_Controller {
 	...
 	public function export_tags(){
 		$query = $this
				->model
				->common_select('equipment_tags.tag_no, equipments.name AS `equipment_name`, `plants`.`name` AS 	`plant_name`, equipment_tags.equipment_use')
		...
		$filename = 'equipment_tags-'.time().'.xlsx';
		$title = 'Equipment Tags';
		$sheetTitle = 'Equipment Tags List';
